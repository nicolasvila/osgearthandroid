...
public:
	...
	if(_options.format().value().compare("kml") == 0){
		wmsFormatToUse = "application/vnd.google-earth.kml+xml";
	}
...
public:
        ...
        ReadResult&        out_response )
    {
	    ...
	    if( mt.compare("application/vnd.ogc.se_xml;charset=UTF-8") == 0 ) mt = "application/vnd.ogc.se_xml";
		...
		if (typeExt.compare("vnd.google-earth.kml+xml") == 0) typeExt = "kml";
                result = osgDB::Registry::instance()->getReaderWriterForExtension( typeExt );
                if ( !result )
                {
                    OE_NOTICE << "WMS: no " << typeExt << " Reader registered; URI=" << createURI(key) << std::endl;
                }
            }
        }
        return result;
	...
	if(_options.format().value().compare("kml") == 0){
		osgEarth::MapNode* mn = (osgEarth::MapNode*) _options.mapNode().value();
		osgEarth::Drivers::KMLOptions* kml_options = (osgEarth::Drivers::KMLOptions*) _options.kmlOptions().value();
		osg::Group* root = (osg::Group*) _options.root().value();

		osg::ref_ptr<SceneGraphChanges> changesQueue = dynamic_cast<SceneGraphChanges*> ( root->getUserData() );
		if(changesQueue && !changesQueue->isIn(_options.getConfig().value("layers"), key.getExtent())){
			osg::ref_ptr<osg::Node> nodekml = osgEarth::Drivers::KML::load( osgEarth::URI(createURI(key)), mn, _dbOptions.get(), *kml_options );
			changesQueue->put(_options.getConfig().value("layers"), key.getExtent(), nodekml);	
			OE_NOTICE << "DEBUG: KML request: " << createURI(key) << std::endl;
		}
		else{OE_NOTICE << "DEBUG: tile rejected, broader tile already in cache" << std::endl;}
		kml = true;
	}
	else{
        	if ( _timesVec.size() > 1 )
			...
			readResult = reader->readImage( buf, _dbOptions.get() );
		
        	        if ( readResult.error())
        	        {
        	            OE_WARN << "WMS: image read failed for " << createURI(key) << readResult.message() << std::endl;
        	        }
        	        else
        	        {
