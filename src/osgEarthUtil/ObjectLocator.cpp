...
ObjectLocatorNode::ObjectLocatorNode()
{
    ...
    osg::ref_ptr<KMLNodeInfo> ni = new KMLNodeInfo();
    setNodeInfo(ni);
}
...
ObjectLocatorNode::ObjectLocatorNode( const Map* map, KMLNodeInfo* nodeInfo ) :
_matrixRevision( -1 )
{
    setNumChildrenRequiringUpdateTraversal( 1 );
    setLocator( new ObjectLocator(map) );
...
void ObjectLocatorNode::setNodeInfo(KMLNodeInfo* nodeInfo){
	_nodeInfo = nodeInfo;
}

KMLNodeInfo* ObjectLocatorNode::getNodeInfo(){
	return _nodeInfo;
}
