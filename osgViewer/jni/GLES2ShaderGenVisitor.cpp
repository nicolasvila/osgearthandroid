...
//then we can completely replace use of gl_FrontLightModelProduct
    ...
    OE_WARN << "ShaderGenCache Vertex shader:\n" << vertstr << std::endl;
    OE_WARN << "ShaderGenCache Fragment shader:\n" << fragstr << std::endl;
...
void GLES2ShaderGenVisitor::apply(osgText::Font &font)
{
    osg::StateSet *stateSet = font.getStateSet();
...
void GLES2ShaderGenVisitor::update(osg::Drawable *drawable)
{
    ...
    if (state->getMode(GL_BLEND) & osg::StateAttribute::ON)
        stateMask |= GLES2ShaderGenCache::BLEND;
