...
Registry::Registry()
{
    // comment out because it was causing problems under OSX - causing it to crash osgconv when constructing ostream in osg::notify().
     OSG_ALWAYS << "Constructing osg::Registry"<<std::endl;
	...
	OSG_ALWAYS << "Registry()	" << kdtree_str <<std::endl;
    if (kdtree_str)
    {
...
void Registry::addReaderWriter(ReaderWriter* rw)
{
    ...
    OSG_ALWAYS << "osg::Registry::addReaderWriter("<<rw->className()<<")"<< std::endl;
...
ImageProcessor* Registry::getImageProcessorForExtension(const std::string& ext)
{
    ...
    OSG_ALWAYS << "Now checking for plug-in "<<libraryName<< std::endl;
    ...
    if (loadLibrary(libraryName)==LOADED)
    {
        ...
        if (!_ipList.empty())
        {
            OSG_ALWAYS << "Loaded plug-in "<<libraryName<<" and located ImageProcessor"<< std::endl;
            return _ipList.front().get();
        }
    }
    return 0;
...
{
    OSG_NOTICE<<"Registry::loadLibrary() -- before lock"<<std::endl;
    OpenThreads::ScopedLock<OpenThreads::ReentrantMutex> lock(_pluginMutex);
    OSG_NOTICE<<"Registry::loadLibrary() -- after lock"<<std::endl;
...
{
	OSG_ALWAYS << "Registry::getReaderWriterForExtension	" << ext << std::endl;
