    ...
    switch (pack(photometric, config))
    {
        ...
        case pack(PHOTOMETRIC_MINISBLACK, PLANARCONFIG_SEPARATE):
            ...
            for (row = 0; row < h; row++)
            {
                ...
                invert_row(currPtr, inbuf, samplesperpixel*w, photometric == PHOTOMETRIC_MINISWHITE, bitspersample);//invert_row(currPtr, inbuf, w, photometric == PHOTOMETRIC_MINISWHITE, bitspersample);//
                currPtr -= format*w;
            }
            break;
